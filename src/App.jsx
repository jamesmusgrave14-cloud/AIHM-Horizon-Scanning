...
import { Shield, Database, Cpu, RefreshCw, Search, TrendingUp, MessageSquare } from "lucide-react";
...
export default function App() {
  ...
  // Derive harm categories dynamically from data.harms
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans antialiased">
      ...
      <main className="p-8 max-w-7xl mx-auto">
        ...
        {/* HARMS MONITOR (your existing view, still grouped by categories) */}
        {activeTab === "harms" && (
        ...
        {/* SIGNALS (emerging trends / clustered) */}
        {activeTab === "signals" && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {signalsVisible.map((s) => {
              const status = triage[s.signal_id] || "new";
              return (
                <article key={s.signal_id} className="border border-slate-100 p-5 rounded-lg bg-slate-50/30 hover:border-slate-300 transition-colors">
                    ...
                    {["new", "watching", "actionable", "archived"].map(st => (
                      <button
                        ...
                        className={`text-[10px] px-3 py-1 rounded-full border transition ${
                          status === st ? "border-slate-900 bg-white" : "border-slate-200 bg-transparent hover:bg-white"
                        }`}
                      >
                        {st}
                      </button>
        ...
        {/* MODEL RELEASES + AIID (your existing list views) */}
