...
function riskAreaChip(label) {
  ...
  if (key.includes("ra10") || key.includes("non-financial")) return "border-teal-200 bg-teal-50 text-teal-900";
...
function subtypeChip(label) {
  ...
  if (key.includes("ransomware") || key.includes("malware") || key.includes("cyber")) return "border-cyan-200 bg-cyan-50 text-cyan-900";
...
export default function App() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("aihm_ui_prefs_v5");
      if (!raw) return;
  ...
  useEffect(() => {
    try {
      localStorage.setItem("aihm_ui_prefs_v5", JSON.stringify({ view, showN, showAiSummaries }));
    } catch {
      // ignore
    }
  ...
  function passesCommon(item, kind) {
    ...
    if (categoryFilter !== "All") {
      const itemCat = getTopRiskArea(item, kind);
      ...
      <div className="max-w-7xl mx-auto px-5 py-5 animate-fadeUp">
        <div className="card p-5 bg-white/80 backdrop-blur border border-slate-200">
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
            <div>
              <div className="flex items-center gap-3">
                ...
                {payload?.last_updated ? (
                  <span className="text-sm text-[var(--muted)] font-mono">updated {String(payload.last_updated).slice(0, 19)}</span>
                ) : null}
              </div>
              ...
              <div className="mt-2 text-xs text-slate-500">
                Top-level taxonomy uses Home Office owned risk areas where available, with subtype labels beneath each item.
              </div>
            ...
            <div className="flex items-center gap-2">
              <button onClick={load} className="pill px-4 py-2 text-sm hover:bg-white transition inline-flex items-center gap-2 bg-white/70" title="Refresh">
                <RefreshCw size={16} className={loading ? "animate-spin" : ""} />
                Refresh
              </button>
              <button onClick={() => setFiltersOpen((v) => !v)} className="pill px-4 py-2 text-sm hover:bg-white transition inline-flex items-center gap-2 bg-white/70" title="Filters">
            ...
            <div className="mt-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {activeFilterChips.map((c) => (
                  <button key={c.k} onClick={() => clearChip(c.k)} className="inline-flex items-center gap-2 text-sm px-3 py-1.5 rounded-full border border-slate-200 bg-white/70 hover:bg-white transition" title="Click to remove" type="button">
                    {c.label}
                    <X size={14} className="text-slate-400" />
                  </button>
                ))}
                <button onClick={clearAllFilters} className="text-sm px-3 py-1.5 rounded-full border border-slate-200 bg-white hover:bg-slate-50 transition" title="Clear all filters" type="button">
                  Clear all
                </button>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
                <div className="lg:col-span-4">
                  ...
                  <div className="relative mt-1">
                    ...
                    <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="titles, risk areas, subtypes, sources…" className="w-full rounded-xl border border-slate-200 bg-white py-2 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200" />
                  </div>
                </div>
                <div className="lg:col-span-2">
                  <label className="text-xs text-[var(--muted)]">Time window</label>
                  <select className="mt-1 w-full pill px-3 py-2 text-sm bg-white" value={timeFilter} onChange={(e) => setTimeFilter(e.target.value)} disabled={!!(dateFrom || dateTo)} title={dateFrom || dateTo ? "Disabled when Date From/To is set" : ""}>
                ...
                <div className="lg:col-span-2">
                  ...
                  <input type="date" className="mt-1 w-full pill px-3 py-2 text-sm bg-white" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} />
                </div>
                <div className="lg:col-span-2">
                  <label className="text-xs text-[var(--muted)]">Date to</label>
                  <input type="date" className="mt-1 w-full pill px-3 py-2 text-sm bg-white" value={dateTo} onChange={(e) => setDateTo(e.target.value)} />
                </div>
                <div className="lg:col-span-2">
                ...
                <div className="lg:col-span-3">
                  <label className="text-xs text-[var(--muted)]">Risk area</label>
                ...
                <div className="lg:col-span-12 flex flex-wrap gap-3 items-center mt-1">
                  ...
                  <button onClick={() => { setDateFrom(""); setDateTo(""); }} className="pill px-3 py-2 text-sm hover:bg-white transition bg-white" title="Clear date range" type="button">Clear dates</button>
                  <button onClick={clearAllFilters} className="pill px-3 py-2 text-sm hover:bg-white transition bg-white" title="Clear all filters" type="button">Clear all</button>
                  <div className="text-xs text-[var(--muted)] font-mono">TIME_WINDOW={limits.TIME_WINDOW || "—"} · RELEASE_TIME_WINDOW={limits.RELEASE_TIME_WINDOW || "—"} · dedupe={meta.dedupe_mode || "—"}</div>
                </div>
              </div>
            </div>
          ) : null}
        ...
        <div className="mt-5 grid grid-cols-1 lg:grid-cols-[260px,1fr] gap-4">
          <aside className="card p-4 bg-white/70 backdrop-blur border border-slate-200 lg:sticky lg:top-4 lg:self-start">
            ...
            <div className="text-xs text-[var(--muted)]">Landing page is Harms. Buckets and filters use Home Office owned risk areas as the top level where items can be mapped cleanly.</div>
          </aside>
...
function NavItem({ icon, label, active, onClick, count }) {
  return (
    <button onClick={onClick} className={`w-full text-left px-3 py-2 rounded-xl mb-1 transition flex items-center justify-between ${active ? "bg-indigo-50 border border-indigo-200" : "hover:bg-slate-50"}`} type="button">
      <span className="inline-flex items-center gap-2 text-sm">
        <span className="text-slate-500">{icon}</span>
        <span className="font-medium">{label}</span>
      </span>
      {count !== null && count !== undefined ? <span className="text-xs font-mono text-slate-500">{count}</span> : null}
    </button>
  );
}
  ...
  return (
    <div className="space-y-4">
      <ViewHeader
        ...
        subtitle="Top-level buckets follow Home Office owned risk areas; cards show subtypes underneath. Use focus chips to reduce scrolling."
      ...
      <div className="card p-4 bg-white/80 backdrop-blur border border-slate-200">
        <div className="flex flex-wrap gap-2 items-center">
          <button type="button" onClick={() => setHarmsFocusCat("All")} className={`text-sm px-3 py-1.5 rounded-full border transition ${harmsFocusCat === "All" ? "bg-rose-700 text-white border-rose-700" : "bg-white/70 hover:bg-white border-slate-200 text-slate-700"}`} aria-pressed={harmsFocusCat === "All"}>All risk areas</button>
          {categories.map((cat) => (
            <button key={cat} type="button" onClick={() => setHarmsFocusCat(cat)} className={`text-sm px-3 py-1.5 rounded-full border ${riskAreaChip(cat)} ${harmsFocusCat === cat ? "ring-2 ring-rose-200" : ""}`} aria-pressed={harmsFocusCat === cat} title="Focus this risk area">
              {cat}
            </button>
          ))}
        </div>
          ...
          {catsToRender.map((cat) => {
            ...
            return (
              <div key={cat} className="border border-slate-100 rounded-2xl overflow-hidden bg-white">
                <button onClick={() => toggleBucket(cat)} className="w-full px-4 py-3 bg-slate-50 flex items-center justify-between" type="button">
                  <div className="flex items-center gap-2 flex-wrap">
                    {isOpen ? <ChevronDown size={16} className="text-slate-500" /> : <ChevronRight size={16} className="text-slate-500" />}
                    <span className={`text-xs px-2 py-1 rounded-full border ${riskAreaChip(cat)}`}>{cat}</span>
                    <span className="text-xs font-mono text-slate-600">shown {items.length}{typeof cov.uk_count === "number" ? ` · UK ${cov.uk_count}` : ""}</span>
                  </div>
                  ...
                  <div className="p-4 bg-white">
                    {showAiSummaries && summaries?.harms_by_category?.[cat] ? (
                      <div className="pill px-3 py-2 mb-3 bg-white/70">
                        <div className="text-xs text-[var(--muted)] flex items-center gap-2"><Sparkles size={14} /> Risk area summary</div>
                      ...
                      {items.map((h, i) => (
                        <article key={`${h.link}-${i}`} className="card-hover border border-slate-100 rounded-2xl p-4 bg-white">
                          <div className="flex items-center justify-between gap-3">
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className={`text-[10px] uppercase tracking-wide font-semibold rounded-full px-2 py-1 border ${riskAreaChip(getTopRiskArea(h, "harms"))}`}>{getRiskCode(h, "harms") || "Risk"}</span>
                              {getSubtype(h, "harms") ? <span className={`text-[10px] rounded-full px-2 py-1 border ${subtypeChip(getSubtype(h, "harms"))}`}>{getSubtype(h, "harms")}</span> : null}
                              <span className="text-xs text-slate-500 font-mono">{fmtDateShort(h.date)}</span>
                            </div>
                            {h.uk_relevance ? <span className="text-xs px-2 py-1 rounded-full border border-indigo-200 bg-indigo-50 text-indigo-800">UK‑relevant</span> : <span className="text-xs px-2 py-1 rounded-full border border-slate-200 bg-slate-50 text-slate-600">Global</span>}
                          </div>
                          <a href={h.link} target="_blank" rel="noreferrer" className="mt-2 block text-sm font-semibold hover:underline">{h.title}</a>
                          <div className="mt-2 text-xs text-slate-500">{h.source}<span className="ml-2 font-mono">uk_score {h.uk_score ?? 0}</span></div>
                          {h.legacy_category && h.legacy_category !== h.category ? <div className="mt-2 text-[11px] text-slate-500">Legacy tag: {h.legacy_category}</div> : null}
                        </article>
                      ))}
                    </div>
                    {!items.length ? <div className="text-sm text-[var(--muted)]">No items in this bucket with current filters.</div> : null}
                  </div>
                ) : null}
              </div>
            );
...
function SignalsView({ items, showAiSummaries }) {
  return (
    <div className="space-y-4">
      <ViewHeader view="signals" title="Signals (clusters)" subtitle="Grouped themes with top-level Home Office risk area mapping and subtype labels." right={<span className="pill px-3 py-1 text-xs font-mono text-slate-600 bg-white/70">{items.length} shown</span>} />
      <div className="card p-4 bg-white/80 backdrop-blur border border-slate-200">
        {!items.length ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {items.map((s) => (
              <article key={s.signal_id} className="card-hover border border-slate-100 rounded-2xl p-4 bg-white">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-[10px] uppercase tracking-wide font-semibold text-sky-700 bg-sky-50 border border-sky-100 rounded-full px-2 py-1">Signal cluster</span>
                    <span className={`text-xs px-2 py-1 rounded-full border ${riskAreaChip(getTopRiskArea(s, "signals"))}`}>{getTopRiskArea(s, "signals")}</span>
                    {getSubtype(s, "signals") ? <span className={`text-xs px-2 py-1 rounded-full border ${subtypeChip(getSubtype(s, "signals"))}`}>{getSubtype(s, "signals")}</span> : null}
                  </div>
                  <span className={`text-xs px-2 py-1 rounded-full border ${confidenceChip(s.confidence_label)}`}>{s.confidence_label || "Low"}</span>
                </div>
                <div className="mt-2 text-sm font-semibold leading-snug">{s.title}</div>
                {showAiSummaries && s.ai_summary ? <div className="mt-2 text-sm text-[var(--muted)] leading-relaxed">{s.ai_summary}</div> : null}
                <div className="mt-2 text-xs text-slate-500 font-mono">latest {fmtDateShort(s.latest_date)} · {s.source_count ?? 0} sources · {s.cluster_size ?? 0} items</div>
                <div className="mt-3 space-y-1">
                  {(s.links || []).slice(0, 5).map((l, i) => {
                    const st = (l.source_type || "news").toLowerCase();
                    const badge = st === "forum" ? "border-fuchsia-200 bg-fuchsia-50 text-fuchsia-900" : "border-slate-200 bg-slate-50 text-slate-700";
                    return (
                      <a key={i} href={l.link} target="_blank" rel="noreferrer" className="block text-sm text-blue-700 hover:underline">
                        <span className={`mr-2 inline-flex text-[10px] px-2 py-1 rounded-full border ${badge}`}>{st.toUpperCase()}</span>
                        {l.source}: {l.title}
                      </a>
                    );
                  })}
...
function ForumsView({ items }) {
  return (
    <div className="space-y-4">
      <ViewHeader view="forums" title="Forums (posts)" subtitle="Forum posts mapped to Home Office risk areas with subtype labels." right={<span className="pill px-3 py-1 text-xs font-mono text-slate-600 bg-white/70">{items.length} shown</span>} />
      <div className="card p-4 bg-white/80 backdrop-blur border border-slate-200">
        {!items.length ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {items.map((f, i) => (
              <article key={`${f.link}-${i}`} className="card-hover border border-slate-100 rounded-2xl p-4 bg-white">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-[10px] uppercase tracking-wide font-semibold text-fuchsia-700 bg-fuchsia-50 border border-fuchsia-100 rounded-full px-2 py-1">Forum post</span>
                    <span className={`text-xs px-2 py-1 rounded-full border ${riskAreaChip(getTopRiskArea(f, "forums"))}`}>{getTopRiskArea(f, "forums")}</span>
                    {getSubtype(f, "forums") ? <span className={`text-xs px-2 py-1 rounded-full border ${subtypeChip(getSubtype(f, "forums"))}`}>{getSubtype(f, "forums")}</span> : null}
                  </div>
                  <span className="text-xs text-slate-500 font-mono">{fmtDateShort(f.date)}</span>
                </div>
                <a href={f.link} target="_blank" rel="noreferrer" className="mt-2 block text-sm font-semibold hover:underline">{f.title}</a>
                <div className="mt-2 text-xs text-slate-500">{f.source}{f.uk_relevance ? <span className="ml-2 text-indigo-700">UK‑relevant</span> : null}</div>
...
function ReleasesView({ items }) {
  return (
    <div className="space-y-4">
      <ViewHeader view="releases" title="Model releases" subtitle="Model releases / model cards / system cards." right={<span className="pill px-3 py-1 text-xs font-mono text-slate-600 bg-white/70">{items.length} shown</span>} />
      <div className="card p-4 bg-white/80 backdrop-blur border border-slate-200">
        {!items.length ? (
          <div className="text-sm text-[var(--muted)]">No model releases match your filters (try widening RELEASE_TIME_WINDOW or removing the risk area filter).</div>
        ) : (
          <div className="space-y-2">
            {items.map((r, i) => (
              <a key={`${r.link}-${i}`} href={r.link} target="_blank" rel="noreferrer" className="block card-hover border border-slate-100 rounded-2xl p-4 bg-white">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] uppercase tracking-wide font-semibold text-indigo-700 bg-indigo-50 border border-indigo-100 rounded-full px-2 py-1">Release</span>
                    <span className={`text-xs px-2 py-1 rounded-full border ${riskAreaChip("Model Releases")}`}>Model release</span>
                  </div>
                  <span className="text-slate-500 font-mono">{fmtDateShort(r.date)}</span>
                </div>
